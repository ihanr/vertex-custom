...
辅种成功后会添加两个标签：新添加的辅种种子加 `Reseed`，实际提供文件的原种加 `Brseed`。只有新种确实添加成功后才会给原种添加 `Brseed`；未命中、仅扫描到候选数据或添加新种失败时都不会添加。两个标签独立重试和确认，一个失败不会阻止另一个继续尝试；最终未确认会在记录中标为“辅种（标签失败）”，不会静默当作成功。
...
辅种逻辑：B 站 RSS 出现种子后，会在“辅种下载器”中逐台查找 **已完成、文件名相同、总大小相同且 info hash 不同** 的种子。命中后，B 站种子会添加到找到数据的**同一台机器**，复用该种子的保存路径、跳过校验，并加上 `Reseed` 标签；提供数据的原种同时加上 `Brseed` 标签。由于已勾选“仅辅种”，没有命中已有数据时不会触发普通下载，也不会给任何原种添加 `Brseed`。
