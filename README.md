...
5. “下载器”和“辅种下载器”必须选择**同一组机器**；应用后先使用“试运行”或观察任务历史，确认匹配到的资源符合预期。

辅种逻辑：B 站 RSS 出现种子后，会在“辅种下载器”中逐台查找 **已完成、文件名相同、总大小相同且 info hash 不同** 的种子。命中后，B 站种子会添加到找到数据的**同一台机器**，复用该种子的保存路径，跳过校验，并加上 `Reseed` 标签。由于已勾选“仅辅种”，没有命中已有数据时不会触发普通下载。
