...
RSS 任务的“分类”下方可填写“标签”。普通下载会在 qB 添加种子时附加该标签；自动辅种的新种会同时保留任务标签与 `Reseed`。多个标签用英文逗号分隔；`Reseed` 和 `Brseed` 是系统标签，不应手工填写。
...
6. 在 RSS 任务中同时勾选 `种子500m-20g` 和 `种子20g-280g` 两条选种规则。
