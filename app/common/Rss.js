...
class Rss {
  ...
  _getClientIds (fitRule = {}) {
    if (Array.isArray(fitRule.clientArr) && fitRule.clientArr.length !== 0) {
      return fitRule.clientArr;
    }
    if (fitRule.client) {
      return [fitRule.client];
    }
    return this.clientArr;
  }

  _selectClient (clientIds) {
  ...
  async _pushTorrent (torrent, _client, fitRule) {
    ...
    if (!this.onlyReseed || !this.autoReseed) {
      ...
      if (!fitRule && this.acceptRules.length !== 0) {
        await util.runRecord('INSERT INTO torrents (hash, name, size, rss_id, link, record_time, record_type, record_note) values (?, ?, ?, ?, ?, ?, ?, ?)',
      ...
      const rule = fitRule || {};
      let savePath = rule.savePath || this.savePath;
      if (savePath) {
