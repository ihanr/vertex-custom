    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'downloader'">
          <span v-if="record.clientArr && record.clientArr.length">{{ downloaderAliases(record.clientArr) }}</span>
          <a-select v-else size="small" :allowClear="true" v-model:value="record.client" style="width: 100%;" @change="modifyRssRuleDownloader(record)">
    ...
    <div style="text-align: left; ">
      <a-form
        ...
        <a-form-item
          label="下载器组"
          name="clientArr"
          extra="非空时仅在此组内按 RSS 任务的排序规则选择下载器；留空则使用单下载器或 RSS 任务下载器">
          <a-select size="small" mode="multiple" :allowClear="true" v-model:value="rssRule.clientArr">
            <template v-for="downloader of downloaders" :key="downloader.id">
              <a-select-option
                :value="downloader.id">
                {{ downloader.alias }}
...
export default {
  ...
  methods: {
    ...
    async modifyRssRule () {
      try {
        const clientArr = (this.rssRule.clientArr || []).filter(item => item);
        const rssRule = { ...this.rssRule, clientArr: clientArr.length ? clientArr : undefined };
        await this.$api().rssRule.modify(rssRule);
    ...
    modifyClick (row) {
      this.rssRule = this.editableRssRule(row);
    },
    cloneClick (row) {
      this.rssRule = { ...this.editableRssRule(row), id: undefined };
    },
    ...
    editableRssRule (row) {
      return {
        ...row,
        clientArr: Array.isArray(row.clientArr) ? [...row.clientArr] : [],
        conditions: (row.conditions || []).map(item => ({ ...item }))
      };
    },
    downloaderAliases (clientArr) {
      return clientArr
        .map(clientId => this.downloaders.find(item => item.id === clientId)?.alias || clientId)
        .join(' / ');
    },
    clearRssRule () {
